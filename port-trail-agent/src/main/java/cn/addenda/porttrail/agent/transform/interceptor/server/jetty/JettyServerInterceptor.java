...
public class JettyServerInterceptor extends AbstractDeduplicationEntryPointInterceptor implements Interceptor {

  private static final PortTrailLogger log =
          AgentPortTrailLoggerFactory.getInstance().getPortTrailLogger(JettyServerInterceptor.class);
  ...
  protected EntryPoint entryPoint(String detail) {
    return EntryPoint.of(EntryPointType.SERVER_JETTY, detail);
  }

  private static final ThreadLocal<Deque<String>> deduplicationStack = new ThreadLocal<Deque<String>>() {
    @Override
    public String toString() {
      return getClass().getName() + "@" + JettyServerInterceptor.class.getName() + "@" + Integer.toHexString(hashCode());
    }
  };
